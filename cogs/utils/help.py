...
class GroupHelpPageSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: HeronPages, commands: List[commands.Command]):
        embed = discord.Embed(
            title=self.title,
    ...
    def format_page(self, menu: HelpMenu, page: Any):
        ...
        if self.index == 0:
            embed.add_field(
                name="About Heron",
                value=(
                    "Insert here whatever you want to say about tte bot\n"
                    "Heron is also open source. You can see the code on [GitHub](https://github.com/real-zephex/discord-bot-1)"
                ),
                inline=False,
            )
        elif self.index == 1:
...
class HeronHelp(commands.HelpCommand):
